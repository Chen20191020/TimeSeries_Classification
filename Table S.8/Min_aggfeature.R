# choosing the min 
sim_func_auto_faster_min = function(n, c_setting, c_1, b_1, c_2, b_2, train_class1, train_class2, test_class1, test_class2){
  # using auto.select 
  ...
  for(num_obs in 1:total_num_train){
    if(num_obs <= train_class1){
      b_end = b_x[num_obs]
      b_start = min(b_end - b_star_x + 1, b_star_x)
      ...
      b_start = min(b_end - b_star_y + 1, b_star_y)
      D_j_list = unlist(lapply(coeffi_list[[num_obs]], function(x) max(x) - min(x))) 
      ma_diff[num_obs]  = max(D_j_list[b_start:b_end])
    }
  }
  ...
  for(num_obs in 1:total_num_test){
    b_end = b_z[num_obs]
    b_start = min(b_end - b_star_z + 1, b_star_z)
    D_j_list = unlist(lapply(coeffi_list[[num_obs]], function(x) max(x) - min(x))) 
...
for(case in 1:5){
  ...
  for(j in 1:500){
    res[j] = sim_func_auto_faster_min(1600, case, 1:5, 1:4, 1:5, 1:4, 100, 100, 25, 25)
  }
...
sim_func_auto_faster_min_realdata = function(Train_list, Testing_list){
  # using auto.select 
...
# classification  
print(sim_func_auto_faster_min_realdata(X_li_train, X_li_test))
