...
for i in range(repeats):
    ts_class1_train = [generate_nAR2_class1(n_elements, 1) for _ in range(n_iterations_class1)]
    ts_class2_train = [generate_nAR2_class2(n_elements, 1) for _ in range(n_iterations_class2)]
    simu_eps1_train = np.array(ts_class1_train + ts_class2_train).reshape(n_iterations_class1+n_iterations_class2, 1, n_elements)

    ts_class1_test = [generate_nAR2_class1(n_elements, 1) for _ in range(n_iterations_t)]
    ts_class2_test = [generate_nAR2_class2(n_elements, 1) for _ in range(n_iterations_t)]
...
for case_name, gen_func_1, gen_func_2 in cases:
    ...
    for idx in range(repeats):
        ...
        arsenal = Arsenal()
        arsenal.fit(eps1_train, Y_label)
        y_pred_multirock = arsenal.predict(eps1_test)
