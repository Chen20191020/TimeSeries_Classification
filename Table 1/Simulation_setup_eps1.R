...
generate_nAR1_class1 = function(n, v){
  ...
  for(i in 1:n){
    if(i == 1){
      ts[i] = 0.4*(cos(2*pi*(i/n))+1)*x_ini  + rnorm(1, 0, 1/v) #  
    } else{
      ts[i] = 0.4*(cos(2*pi*(i/n))+1)*ts[i-1] + rnorm(1, 0, 1/v) #  
    }
  }
  return(ts)
}
...
generate_MA2_class2 = function(n, v){
  ...
  for(i in 3:(n+2)){
    ts[i-2] = 0.3*epsilon[i-2] + 0.4*epsilon[i-1]  + epsilon[i] #  
  }
  return(ts)
}
...
generate_nAR1.2_class1 = function(n, v){
  ...
  for(i in 1:n){
    if(i == 1){
      ts[i] =  (1.5*sin(2*pi*(i/n)))*exp(-(i/n)*x_ini^2)  + rnorm(1, 0, 1/v) #  
    } else{
      ts[i] = (1.5*sin(2*pi*(i/n)))*exp(-(i/n)*ts[i-1]^2) + rnorm(1, 0, 1/v) #  
    }
  }
  return(ts)
}
...
generate_nAR1.2_class2 = function(n, v){
  ...
  for(i in 1:n){
    if(i == 1){
      ts[i] =  (0.5*cos(2*pi*(i/n)))*exp(-(i/n)*-x_ini^2)  + rnorm(1, 0, 1/v) #  
    } else{
      ts[i] =  (0.5*cos(2*pi*(i/n)))*exp(-(i/n)*ts[i-1]^2) + rnorm(1, 0, 1/v) #  
    }
  }
  return(ts)
}
...
generate_AR2.2_class1 = function(n, v){
  ...
  for(i in 1:n){
    if(i == 1){
      ts[i] =  0.2*w[1] + 0.2*sin(2*pi*(i/n))*w[2] + w[i+2]
    } else if(i == 2){
      ts[i] =  0.2*w[2]  + 0.2*sin(2*pi*(i/n))*ts[i-1] + w[i+2]
    } else {
      ts[i] =  0.2*ts[i-2] + 0.2*sin(2*pi*(i/n))*ts[i-1] + w[i+2]
    }
  }
  return(ts)
}
...
generate_AR2.2_class2 = function(n, v){
  ...
  for(i in 1:n){
    if(i == 1){
      ts[i] =  (0.2*sin(2*pi*(i/n)))*w[1] + 0.2*w[2] + w[i+2]
    } else if(i == 2){
      ts[i] =  (0.2*sin(2*pi*(i/n)))*w[2]  + 0.2*ts[i-1] + w[i+2]
    } else {
      ts[i] =  (0.2*sin(2*pi*(i/n)))*ts[i-2] + 0.2*ts[i-1] + w[i+2]
    }
  }
  return(ts)
}
...
generate_nAR2_class1 = function(n, v){
  ...
  for(i in 1:n){
    if(i == 1){
      ts[i] =  0.2*exp(-(i/n)*w[1]^2) + 0.2*(sin(2*pi*(i/n))+1)*(1/(w[2]+1)) + w[i+2]
    } else if(i == 2){
      ts[i] =  0.2*exp(-(i/n)*w[2]^2)  + 0.2*(sin(2*pi*(i/n))+1)*(1/(ts[i-1]+1)) + w[i+2]
    } else {
      ts[i] =  0.2*exp(-(i/n)*ts[i-2]^2) + 0.2*(sin(2*pi*(i/n))+1)*(1/(ts[i-1]+1)) + w[i+2]
    }
  }
  return(ts)
}
...
generate_nAR2_class2 = function(n, v){
  ...
  for(i in 1:n){
    if(i == 1){
      ts[i] =  0.3*(sin(2*pi*(i/n))+1)*w[1] + 0.2*exp(-(i/n)*w[2]^2) + w[i+2]
    } else if(i == 2){
      ts[i] =  0.3*(sin(2*pi*(i/n))+1)*w[2]  + 0.2*exp(-(i/n)*ts[i-1]^2) + w[i+2]
    } else {
      ts[i] =  0.3*(sin(2*pi*(i/n))+1)*ts[i-2] + 0.2*exp(-(i/n)*ts[i-1]^2) + w[i+2]
    }
  }
  return(ts)
}
...
generate_ARbump_class2 = function(n, v, h = 0.02){
  ...
  for(i in 1:n){
    ...
    if(i == 1){
      ts[i] = 0.3 * exp(-((i/n - 1/2)^2) / (2 * h^2)) * x_ini + rnorm(1, 0, 1/v)
    } else{
      ts[i] = 0.3 * exp(-((i/n - 1/2)^2) / (2 * h^2)) * ts[i-1] + rnorm(1, 0, 1/v)
    }
  }
