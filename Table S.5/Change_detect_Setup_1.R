...
detect_change_points = function(data, c_candi = 1:2, d_candi = 1, m = 1000, delta = 0.0001){
...
for(model_case in 1:4){
  ...
  for(j in 1:500){
    ...
    for(i in 1:50){
      if(i <= 25){
        ts = generate_illAR1_class1(10000, 1, 0, 0, t_12[model_case])
        res[i] = detect_change_points(ts, 1:c_m[model_case], 1)
      } else{
        ts = generate_illAR1_class2(10000, 1, 0, t_21[model_case], 1)
        res[i] = detect_change_points(ts, 1:c_m[model_case], 1)
      }
    }
