...
for(ind_del in 1:length(n_del)){
  ...
  n_train_2 = n_del[ind_del]*n_train_1
  ...
  for(repea in 1:500){
    ...
    for(j in 0:J){
      aux = rep(0, n)
      for(i in 1:n_train_1){
        aux = aux + accessD(X_li_s[[i]]$S, level = j)
      }
      
      S_1j[[j+1]] = aux/n_train_1
    }
    
    S_2j = list()
    for(j in 0:J){
      aux = rep(0, n)
      for(i in (n_train_1+1):n_train_total){
        aux = aux + accessD(X_li_s[[i]]$S, level = j)
      }
      
      S_2j[[j+1]] = aux/n_train_2
    }
    ...
    for(j in 0:J){
      ...
      sigma_jk[[j+1]] = aux/n_train_total
    }
    ...
    for(i in 1:n_test){
      if(i <= (n_test/2)){
        ts_classi = generate_AR1_test_class1(n, 1, del)
        ...
        ts_classi = generate_AR1_test_class2(n, 1, del)
        s_classi = ewspec(ts_classi)
        D_1 = 0
        D_2 = 0
        for(num in 1:top_n){
