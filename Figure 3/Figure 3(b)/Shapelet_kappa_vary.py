...
delta_values = np.arange(1, 5.5, 0.5)
...
for ind_delta in delta_values:
    ...
    n_iterations_class2 = ind_delta*n_iterations_class1
    arr = []
    # Add 252 "1"s
    for _ in range(n_iterations_class1):
      arr.append('1')
    for _ in range(int(n_iterations_class2)):
      arr.append('0')
    Y_label = np.array(arr, dtype='<U1')
    for i in range(repeats):
        # Generate training and testing datasets
        ts_class1_train = [generate_AR1_class1(n_elements, 1, 0.2) for _ in range(n_iterations_class1)]
        ts_class2_train = [generate_AR1_class2(n_elements, 1, 0.2) for _ in range(int(n_iterations_class2))]
        simu_eps1_train = np.array(ts_class1_train + ts_class2_train).reshape(int(n_iterations_class1 + n_iterations_class2), 1, n_elements)

        ts_class1_test = [generate_AR1_class1(n_elements, 1, 0.2) for _ in range(n_iterations_t)]
        ts_class2_test = [generate_AR1_class2(n_elements, 1, 0.2) for _ in range(n_iterations_t)]
        simu_eps1_test = np.array(ts_class1_test + ts_class2_test).reshape(50, 1, n_elements)
        
        stsf = ShapeletTransformClassifier(estimator=RandomForestClassifier(ccp_alpha=0.01), n_shapelet_samples = 10)
        stsf.fit(simu_eps1_train, Y_label)
        y_pred_STSForest = stsf.predict(simu_eps1_test)
