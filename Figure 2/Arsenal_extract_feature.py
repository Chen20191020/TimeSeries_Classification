...
# fit 
Arsenal = Rocket(n_kernels=2000)  # Arsenal uses 2000 kernels and ensemble
Arsenal.fit(simu_train)
X_train_transform = Arsenal.transform(simu_train)

classifier = RidgeClassifierCV(alphas=np.logspace(-3, 3, 10))
classifier.fit(X_train_transform, Y_label)

# Feature used in boxplot
X_test_transform = Arsenal.transform(simu_test)
